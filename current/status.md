...
- **PostHog analytics (FE):** `posthog-js` + `@posthog/react`; client in `src/lib/posthog.ts`. Super properties: `session_id` (boot), `user_id` (after sign-in), `plan_id` (on plan routes). Custom events **currently fired:** `user_signed_in`, `user_signed_out`. Additional `track*` events are defined in `src/core/analytics.ts` but not yet wired from hooks/UI — see `guides/frontend.md` § Analytics. Autocapture on (PostHog defaults). Disabled when token is placeholder `"token"`. `VITE_POSTHOG_MOCK=true` uses `src/lib/mock-posthog.ts`. Config: `VITE_PUBLIC_POSTHOG_*` env vars.
