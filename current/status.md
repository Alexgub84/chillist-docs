...
| AI Suggestions | `POST /plans/:planId/ai-suggestions/stream`                                       | SSE streaming variant — fires one AI call per category (personal_equipment, group_equipment, food) in parallel and streams results as each completes. Events: `suggestions` (per-category), `error`, `done`. 3 AI usage log rows per request with shared `streamRequestId` in metadata for per-category analytics |
