...
| Internal       | `GET /api/internal/plan-tags`                                                     | Full plan tag taxonomy (v1.3) for chatbot use. Served from static JSON file (`src/data/plan-creation-tags.json`). Requires `x-service-key` only.                                                                            |
| Plan Tags      | `GET /plan-tags`                                                                  | Full plan tag taxonomy v1.3 (tier1 archetypes, universal flags with contradiction pairs, tier2 axes, tier3 specifics with `multi_select_parents`, item generation bundles). Served from static JSON file. Requires JWT.      |
