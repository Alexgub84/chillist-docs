...
> **FE version:** 1.35.0 — **Plan tags from API** (tag taxonomy from `GET /plan-tags`). **PostHog production proxy:** same-origin `/ingest` via Cloudflare Pages Function → US PostHog hosts; `ui_host` for PostHog UI; dev still uses `VITE_PUBLIC_POSTHOG_HOST`. See `guides/frontend.md` § PostHog.
