...
A user with the **owner** role on the plan (the creator and any co-owners) can edit or delete the plan. The **original creator** is still what `createdByUserId` points to; some responses only expose full **invite tokens** to that user when listing participants, while every **owner** row has the same day-to-day management permissions in the app. Platform admins can also delete any plan they are not a participant of — if an admin is an invited participant on a plan, they see the leave button instead of the admin delete button on that card.
...
Plans can have **multiple** participants with `role: "owner"` (co-owners). The label **"Make owner"** in the app **adds** another owner; it does not replace or demote the previous one. **Any** current owner on the plan (not only the user in `createdByUserId`) can call `PATCH /participants/:participantId` with body `{ "role": "owner" }` only (no other fields in the same request). The backend **does not** change `plans.createdByUserId` or `plans.ownerParticipantId` when adding a co-owner. If the target has not accepted the invite yet, the API returns **400** with `code: participant_not_linked`. If the caller is not an owner on that plan, **403** with `code: not_plan_owner`. If the body mixes `role: owner` with other fields, **400** with `code: owner_promotion_invalid_body`. Earlier confusion with "passing" or **sole** ownership transfer is deprecated as a product story — that behavior is **not** what co-owner promotion does.
...
| Internal       | `POST /api/internal/plans`                                                        | Create plan for `x-user-id`; optional body `ownerPreferences` for owner participant RSVP, headcount, dietary text. `201` `{ plan: { id, name, date } }`. Requires `x-service-key` + `x-user-id`.                                                                                                |
