...
**Date:** 2026-04-22
**Problem:** (1) **Contract bug:** `PATCH /participants/:id` with `role: "owner"` failed in production with `FST_ERR_VALIDATION` because `UpdateParticipantBody` did not allow `owner` in the `role` enum — so the route handler never ran. (2) **Product confusion:** We sometimes described the action as "transfer ownership" / "passing" the plan to one other person. The intended **MVP** is **multiple plan owners**: several participants can have `role: owner` at the same time, with the same owner capabilities for participants, items, and join requests. (3) **Implementation drift:** A **sole-transfer** path (demote the previous `owner` row, rewrite `plans.ownerParticipantId` and `createdByUserId`) matches a different product and is easy to conflate with "make owner" in the UI.
**Solution:** `role` in the PATCH body includes `owner`. The handler **adds a co-owner**: it sets the target `participants` row to `owner` and does **not** demote existing owners; it does **not** change `plans.createdByUserId` or `plans.ownerParticipantId` (those stay **creator/primary** metadata; the authoritative set of "who is an owner" is all participants on the plan with `role === 'owner'`). The caller must already be an `owner` on that plan (any co-owner, not only the user who created the plan in `plans.createdByUserId`). Use stable error `code`s: `participant_not_linked` (400), `not_plan_owner` (403), `owner_promotion_invalid_body` (400).
**Lesson:** Distinguish **granting the owner role** (N owners) from **reassigning plan creator fields** (one row in `plans`). If the product allows co-owners, add integration tests that `PATCH` `role: owner` and assert the previous owner row **stays** `owner` and `plans.*` is unchanged. Keep OpenAPI and user-facing copy aligned (avoid "transfer" if you mean "add another owner"). Some routes still key off `createdByUserId` for **invite token visibility** on list responses — that is separate from the owner **role**; document when touching those flows.
