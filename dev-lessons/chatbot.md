...
**Decision:** Changed the create-plan flow to: (1) do not require plan dates or location before `createPlan` — those are optional plan fields; offer or add them after the plan exists unless the user already gave them; (2) before `createPlan`, have a clear title and either owner preference answers (group size, food, allergies) or a clear skip — brief questions if needed; if the user gives title + prefs in one message, call `createPlan` in that turn; (3) always set `ownerPreferences.rsvpStatus: "confirmed"`; (4) after creation, share the plan link and offer dates/location if missing; (5) when user asks about adding items (out of v1 scope), say "You can add items on the plan page" with the link — soft wording, no "use the app" phrasing. Extended `POST /api/internal/plans` to accept optional `ownerPreferences` object (BE issue #207). System prompt and `createPlan` tool description use the same ordering so the model is not told both "as soon as you have a title" and "ask first" without scope (dates/location vs preferences).
**Reason:** Faster flow for users (no blocking on optional plan metadata), better data capture (owner prefs at creation), softer UX (no app-pushing), unambiguous LLM instructions.
