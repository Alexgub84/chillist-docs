...
### [API] Shipped FE with a `DietaryMember` shape the backend cannot accept

**Date:** 2026-04-23
**Problem:** Implementing “multi-select food preferences + new diet tags (`no_fish`, `no_pork`)” I edited [`src/core/openapi.json`](../../chillist-fe/src/core/openapi.json) **locally** to add `diets: enum[]`, make `diet` optional, and extend the diet enum, then ran `npm run api:types` and built the FE against that shape. The file is generated by `scripts/fetch-openapi.sh` from **chillist-be** (`docs/openapi.json`); the real backend still enforces the old contract — `diet` required, single enum of 10 values, no `diets`. So a signed-in / guest write that went through the new UI would either be **rejected by BE validation** (unknown `diets`, missing `diet`, unknown enum value) or silently lose the new tags. The local mock (`api/mock.ts` + `api/server.ts`) happily accepted it, so dev felt green while prod would 400. User spotted it and reverted `openapi.json`; the generated `api.generated.ts` was left in the “aspirational” state until BE catches up.
**Root cause:** Treated `src/core/openapi.json` as a source of truth I could extend for a contract change. It isn’t — it’s a **cached copy** of the BE spec. Any change to the server contract must land in **chillist-be** (OpenAPI + Fastify/AJV validation + persistence) first, then the FE pulls it via `npm run api:sync`. Also: the local mock server doesn’t validate against the canonical enum list strictly enough to surface the drift.
**Solution:** Revert `src/core/openapi.json` to the upstream shape (keep `diet` required, drop `diets` and `no_fish`/`no_pork`). File a BE issue ([`chillist-docs/specs/be-issue-dietary-members-multi-select-and-new-diets.md`](../specs/be-issue-dietary-members-multi-select-and-new-diets.md)) for the real contract change. On the FE keep the multi-select UI and new labels behind the existing `diets[]` data path but keep sending the legacy `diet` for write endpoints until BE ships; once BE ships, re-run `npm run api:sync` and regenerate types.
**Prevention:**
- **Never edit `src/core/openapi.json` by hand.** It is generated — treat it as read-only. For any contract change, open a **chillist-be** issue first and let `npm run api:sync` bring the new shape in.
- When a FE task says “add field X to API”, the plan’s first todo must be a **BE** todo (OpenAPI + validation + migration) — not `openapi.json` edit.
