...
| `src/lib/posthog.ts`  | `posthog-js`            | Real init when token is set and not `"token"`. `VITE_POSTHOG_MOCK=true` exports `src/lib/mock-posthog.ts` (no network). Production uses `api_host: '/ingest'` (same-origin proxy); dev uses `VITE_PUBLIC_POSTHOG_HOST`. |

### PostHog — local dev, staging, and toolbar

- **Production proxy:** Browser sends analytics to `https://<your-site>/ingest/*`. A Cloudflare Pages Function at `chillist-fe/functions/ingest/[[path]].ts` proxies to `us.i.posthog.com` / `us-assets.i.posthog.com`. No GitHub variable change is required when the public domain changes — `src/lib/posthog.ts` sets `/ingest` automatically when `import.meta.env.PROD` is true. `ui_host` is fixed to `https://us.posthog.com` for toolbar / UI links (US cloud).
...
`functions/ingest/[[path]].ts` proxies `/ingest/*` → `us.i.posthog.com` (events, session recording API) and `us-assets.i.posthog.com` (`/static/*`, `/array/*`). Same-origin requests avoid ad blockers and Cloudflare blocking direct calls to PostHog from the production origin.

Flow: **browser** → `GET/POST https://<your-domain>/ingest/...` → **Pages Function** → PostHog US hosts. `X-Forwarded-For` is set from `CF-Connecting-IP` so geo is accurate.

The proxy deploys with every Pages build — no separate Wrangler project or deploy step. Changing the public domain (e.g. `*.pages.dev` → custom domain) does **not** require updating `VITE_PUBLIC_POSTHOG_HOST` in GitHub; production `api_host` stays the relative path `/ingest`.

**Verify after deploy:** In DevTools → Network, confirm PostHog traffic goes to your origin under `/ingest/` (200 responses). In PostHog Live / Activity, new sessions should show `environment: production` and production URLs in `$current_url`.
