...
- **All quality test session IDs must use the `qt-` prefix** (e.g. `"qt-mark-done"`, `"qt-he-bulk"`). The tee logger in `createQualityLoggerSetup()` tags all entries with `source='quality-test'`, enabling filtering: `SELECT * FROM chatbot_ai_usage WHERE source = 'quality-test'`.
