...
- When a category has no items but the plan already has items elsewhere, the empty category section can open bulk add scoped to that category (starts at subcategory selection) on the plan page and manage-items view.
...
- **Buying List** — default tab; items still to buy (pending + unassigned). **Packing List** — purchased but not packed. **All Items** — full list **excluding** canceled rows; canceled items behave as if deleted on the frontend and never surface in any tab.
...
**Inline editing:** Tap any item to edit its quantity, unit, or status directly. Equipment items always use "pieces" as the unit. Marking an item **canceled** (via the × button) is available on every editable row, including unassigned items — canceling an unassigned row creates a canceled assignment entry for the current viewer. Canceled items are hidden from every frontend surface: the Buying, Packing, and All tabs; the bulk-add wizard's "already in plan" pre-selection; and the AI suggestion's "existing items" set. Re-adding a canceled item via bulk-add reactivates the existing row (viewer's entry flips back to `pending`) instead of creating a duplicate.
...
- **Header brand logo** — routes to `/plans` for authenticated users and to `/` for guests.
...
- Open the admin dashboard at `/admin/dashboard` with tabs: **All Plans**, **AI Usage**, and **Chatbot AI**.
...
- `POST /api/internal/plans` → `{ plan: { id, name, date } }` (`201`; x-service-key + `x-user-id`; owner PII resolved server-side; optional `ownerPreferences` for owner RSVP, headcount, dietary text — for WhatsApp chatbot)
...
| AI item suggestions       | Done        | **BE:** `POST /plans/:planId/ai-suggestions/:category` with `category` ∈ {`food`,`group_equipment`,`personal_equipment`} (JWT). FE sends **three parallel** requests per generate; optional header **`X-Generation-Id: <uuid>`** on all three for correlated `ai_usage_logs`. Optional body `{ "subcategories"?: string[] }` per category. Response `{ suggestions, aiUsageLogId, generationId }`; 502 on AI failure. `name`, `subcategory`, `reason` follow `defaultLang` (en/he/es); `category` and `unit` stay English enums. AI may invent plan-specific subcategory labels. **FE (issue #195):** "Suggest items with AI" on plan creation step 5 (above bulk library). On the plan page, Manage Items route, and plan creation wizard, **admin-only:** inline button beside the Items heading and floating-menu action (non-admin users do not see these). Calls AI endpoint, preview modal nested by category then subcategory (category- and subcategory-level select/deselect all), item checkboxes, quantity, reason text; confirms via `POST /plans/:planId/items/bulk`. `personal_equipment` uses `isAllParticipants`. After confirming AI suggestions during plan creation, the wizard closes and navigates to the plan. Errors 503 (retry), 404, 500. Mock server + E2E fixtures include the AI route. FE subcategory i18n refactor: issue #196. See [ai-item-generation.md](ai-item-generation.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| PostHog analytics (FE)    | Done        | `posthog-js` + `@posthog/react`. Client `src/lib/posthog.ts`; super props + identify/reset. Wired: `user_signed_in`, `user_signed_out`, `item_updated`, `profile_completed`, `invite_link_copied`, `invite_claimed`, `join_request_submitted`, `participant_added`, `participant_left`, `ownership_transferred`, `join_request_moderated`. Mock client (`mock-posthog.ts`) accumulates events for dev logging and test assertions. See `guides/frontend.md` § Analytics.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
