...
| Share link | Done | Invite token per participant, public `GET /plans/:planId/invite/:inviteToken` endpoint. FE: copy/share buttons in Manage Participants modal and Group Details section. Invite landing page at `/invite/:planId/:inviteToken` — read-only plan view with auth-aware CTA: unauthenticated users see "Sign in to join" / "Create an account" linking to `/signin?redirect=/plan/:planId`; authenticated users see "Go to plan" linking directly to `/plan/:planId`. Sign-in and sign-up pages support `?redirect` search param for post-auth navigation. Mock server + E2E tests. Issue #60, #101. **Invite claim flow (issue #109 — fixed):** Email auth awaits `claimInvite()` before navigation. OAuth redirects to invite page (public, works without claim), AuthProvider claims in background. Guest preferences modal stays on invite page (no redirect to auth-gated route). Claiming nullifies the invite token — the invite link stops working after claim, and the user accesses the plan via JWT going forward. **Invite token security:** `inviteToken` is deleted from DB on claim (`inviteStatus` set to `accepted`). API responses only include `inviteToken` for the plan owner (and admins); non-owners always receive `null`. `inviteStatus` is now a required field on all participant responses (`pending` / `invited` / `accepted`). FE issue #115 tracks displaying invite status per participant. **Remaining:** `PATCH /plans/:planId/invite/:inviteToken/preferences` endpoint not yet implemented in BE (guest preferences save returns 404 in production). |
...
- **Participant**
  - `participantId`, `displayName`, `name`, `lastName`, `role` ("owner" | "participant" | "viewer"), `inviteStatus` ("pending" | "invited" | "accepted"), optional: `avatarUrl`, `contactEmail`, `contactPhone`, `adultsCount`, `kidsCount`, `foodPreferences`, `allergies`, `notes`
  - `inviteToken` is present only for the plan owner; nullified on claim
...
- **Security:** CORS restriction + Supabase JWT verification via JWKS (all protected routes). Invite routes use token-based auth. See [Backend Guide — Security](../guides/backend.md#security).
...
- **Security:** CORS + Supabase JWT verification via JWKS (asymmetric keys, no secrets stored). All protected routes require JWT. Invite routes use token-based auth. See [Backend Guide — Security](../guides/backend.md#security).
