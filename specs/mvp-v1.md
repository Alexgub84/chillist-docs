...
- `GET /plan-tags` → full bundled taxonomy JSON (public, no auth; same file as internal route, including `structural_contract`, `design_principles`, `changelog`). **FE: `usePlanTags` hook, `staleTime: Infinity`, Zod-validated via `planTagsSchema`**
- `GET /api/internal/plan-tags` → same payload as `GET /plan-tags` (x-service-key required, no x-user-id — for WhatsApp chatbot)
