...
- **Hero section** with campfire photo, headline, subtitle, and auth-aware CTA.
- **"How it works"** — 3-step quick intro (Create a plan → Add gear and food → Track together) with mobile screenshots.
- **"Everything you need"** — 7-feature showcase in story order: Plans Dashboard, Manage Your Group, AI Packing Lists, Easy Item Management, Live Progress Tracking, Expenses & Settlement, WhatsApp Integration. Each feature has a mobile screenshot (EN + HE), title, and description.
- **Bottom CTA** for plan creation / sign-up.
- Scroll-reveal animations, language-aware screenshots (`-he` suffix for Hebrew), auth-aware CTAs. Screenshot script: `npm run screenshots`.
...
- **Item**
  ...
  - `source` ("manual" | "ai_suggestion") — how the row was added; `ai_suggestion` when created from an accepted AI suggestion
  - `aiSuggestionId` (nullable UUID, FK to `ai_suggestions`) — set when `source` is `ai_suggestion`
...
- **AiSuggestion**
  - `id`, `aiUsageLogId` (nullable FK to `ai_usage_logs`), `planId` (FK to plans), `name`, `category`, optional `subcategory`, `quantity` (numeric), `unit`, optional `reason`, `status` ("suggested" | "accepted"), optional `itemId` (FK to items — set when the user accepts and an item row is created)
  - One row per generated suggestion; multiple generations per plan each get their own `ai_usage_logs` row and linked suggestion rows
  - Timestamps: `createdAt`
...
- `POST /plans/:planId/items` → `201 Item` (JWT required — with `assignmentStatusList` for per-participant tracking; optional `aiSuggestionId` to link an accepted AI suggestion)
- `POST /plans/:planId/items/bulk` → bulk create items (JWT required; each entry may include `aiSuggestionId` when adding from the AI preview)
...
| AI item suggestions       | Done        | **BE:** `POST /plans/:planId/ai-suggestions` (JWT, no body; context from plan). Persists rows to `ai_suggestions` and returns `aiUsageLogId` plus each suggestion's `id` for linking on accept. `name`, `subcategory`, and `reason` follow `defaultLang` (en/he/es); `category` and `unit` stay English enums. AI may invent plan-specific subcategory labels. Item create + bulk accept optional `aiSuggestionId` (same plan, status `suggested`); created items include `source` and `aiSuggestionId`. **FE (issue #195):** "Suggest items with AI" … confirms via `POST /plans/:planId/items/bulk` — **should send `aiSuggestionId` per selected row** for analytics (see FE issue for tracking + admin reports). `personal_equipment` uses `isAllParticipants`. After confirming AI suggestions during plan creation, the wizard closes and navigates to the plan. Errors 503 (retry), 404, 500. Mock server + E2E fixtures include the AI route. FE subcategory i18n refactor: issue #196. See [ai-item-generation.md](ai-item-generation.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Home / Landing page       | Done        | Hero + 3-step "How it works" + 7-feature showcase (Plans Dashboard, Group Management, AI Lists, Item Management, Progress Tracking, Expenses, WhatsApp). Mobile screenshots EN/HE, scroll-reveal animations, auth-aware CTAs. Screenshot script: `npm run screenshots`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
