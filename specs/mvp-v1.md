...
| PostHog analytics (FE)    | Done        | `posthog-js` + `@posthog/react`. Client `src/lib/posthog.ts`; `initAnalytics()` + identify/reset + super props (`session_id`, `user_id`, `plan_id`). Custom events wired: `user_signed_in`, `user_signed_out`. More `track*` helpers in `src/core/analytics.ts` (plan/items/AI/invite/expense) — wire as features need them. See `guides/frontend.md` § Analytics. |
